...
// --- AUTOAKTUALIZACJA BAZY DANYCH (Szkice) ---
...
// ZABEZPIECZENIE PRZED DUPLIKOWANIEM (Wzorzec PRG)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST['action']) && !isset($_POST['download_latest_backup'])) {
    ...
    try {
        if (isset($_POST['update_all_views']) && !empty($_POST['post_views'])) {
            ...
            foreach ($_POST['post_views'] as $pid => $v) { $stmt->execute([(int)$v, (int)$pid]); }
            $_SESSION['admin_msg'] = "<p style='color:green; font-weight:bold;'>Wyświetlenia dla czytelników zostały zaktualizowane we wszystkich wpisach!</p>";
            $should_redirect = true;
        }

        if (isset($_POST['add_post'])) {
            $parent_id = (int)($_POST['parent_id'] ?? 0); $linked_album_id = (int)($_POST['linked_album_id'] ?? 0);
            $is_published = isset($_POST['is_published']) ? 1 : 0; // SZKIC LUB OPUBLIKOWANY
            $slug = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $_POST['title_pl']), '-')) . '-' . substr(uniqid(), -5);
            $pdo->prepare("INSERT INTO posts (title, slug, location, cover_image, content, parent_id, linked_album_id, title_en, content_en, title_it, content_it, title_es, content_es, title_de, content_de, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")->execute([
                $_POST['title_pl'], $slug, $_POST['location'], $_POST['cover_image'], $_POST['content_pl'], $parent_id, $linked_album_id, $_POST['title_en'] ?? '', $_POST['content_en'] ?? '', $_POST['title_it'] ?? '', $_POST['content_it'] ?? '', $_POST['title_es'] ?? '', $_POST['content_es'] ?? '', $_POST['title_de'] ?? '', $_POST['content_de'] ?? '', $is_published
            ]);
        ...
        if (isset($_POST['update_post'])) {
            ...
            if ($post_id > 0) {
                ...
                $stmt = $pdo->prepare("UPDATE posts SET title=?, slug=?, location=?, cover_image=?, content=?, views=?, parent_id=?, linked_album_id=?, title_en=?, content_en=?, title_it=?, content_it=?, title_es=?, content_es=?, title_de=?, content_de=?, is_published=? WHERE id=?");
                $stmt->execute([
                    $title_pl, 
                    $slug, 
                    $_POST['location'] ?? '', 
...
// ODBIÓR WIADOMOŚCI Z SESJI (PO PRZEKIEROWANIU)
...
<body>
    ...
    <div class="grid-2">
        <div class="box" id="containerAddPost">
            ...
            <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="addPostForm">
                ...
                <input type="text" name="location" placeholder="Lokalizacja (wspólna)" required>
        ...
        <div class="box" id="containerEditPost">
            ...
            <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" id="editPostForm">
                ...
                <input type="text" name="location" id="editPostLocation" placeholder="Lokalizacja" required>
    ...
    <div class="box">
        <h2>8. Statystyki i Szybka Edycja Wyświetleń</h2>
        <p style="font-size: 13px; color: #666; margin-bottom: 15px;">Tutaj widzisz wszystkie swoje wyprawy. Zmień główny licznik 👁️ w okienku i zapisz. Szary, prawdziwy licznik widoczny jest <strong>tylko dla Ciebie.</strong></p>
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($csrf_token); ?>">
            <div style="display:flex; flex-direction:column; gap: 8px; max-height: 400px; overflow-y: auto; padding-right: 10px; margin-bottom: 15px;">
                <?php foreach($posts as $p): ?>
                    <div style="display:flex; justify-content:space-between; align-items:center; background:#f8fbff; padding:10px 15px; border:1px solid #cce5ff; border-radius:6px; flex-wrap:wrap; gap:10px;">
                        ...
                        <div style="display:flex; align-items:center; gap:10px;">
                            ...
                            <span title="Wyświetlenia widoczne na blogu dla czytelników" style="font-size:18px; margin-left:10px; cursor:help;">👁️</span>
                            <input type="number" name="post_views[<?php echo $p['id']; ?>]" value="<?php echo (int)$p['views']; ?>" style="width:100px; margin:0; padding:8px; border:1px solid #b8daff; text-align:center; font-weight:bold; border-radius:4px;">
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
            <button type="submit" name="update_all_views" class="btn-green" style="width:100%; font-size:16px;">💾 Zapisz wszystkie zmiany oficjalnych wyświetleń</button>
        </form>
    </div>
    ...
    <script>
        ...
        async function runAutoTranslate(mode, event) {
            ...
            let titlePl, contentPl, titleTarget, contentTarget;
            if (mode === 'add') { 
